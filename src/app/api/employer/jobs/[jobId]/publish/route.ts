...
export async function POST(request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  ...
  const result = await callAuthenticatedApi(`/api/v1/organizations/${organizationId}/jobs/${jobId}/publish`, { method: "POST" });
  if (!result.response.ok) return NextResponse.json({ message: await apiErrorMessage(result.response, "Job could not be published.") }, { status: result.response.status });
