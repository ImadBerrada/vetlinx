...
export async function POST(request: Request, context: RouteContext<"/api/reviews/[requestId]/decision">) {
  const { requestId } = await context.params;
  const body = (await request.json().catch(() => null)) as { action?: keyof typeof endpoints; reason?: string } | null;
  if (!body?.action || !(body.action in endpoints)) {
    return NextResponse.json({ message: "A valid decision is required." }, { status: 400 });
  }
  const result = await callAuthenticatedApi(
    `/api/v1/verification-reviews/${encodeURIComponent(requestId)}/${endpoints[body.action]}`,
    {
      method: "POST",
  ...
  if (!result.response.ok) {
    return NextResponse.json(
      { message: await apiErrorMessage(result.response, "The decision could not be recorded.") },
